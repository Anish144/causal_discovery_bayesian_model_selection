...
def train_gpflux(
    ...
    kernel = gpflow.kernels.SquaredExponential(
        lengthscales=[0.05, 0.2], variance=1.0
    )
    ...
    likelihood_layer = gpflux.layers.LikelihoodLayer(
        gpflow.likelihoods.Gaussian(0.01)
    )
    ...
    history = model.fit(
        {"inputs": tf.cast(x, tf.float64), "targets": tf.cast(y, tf.float64)},
        epochs=int(20e3),
        verbose=0,
        batch_size=1000,
        shuffle=False,
    ...
    samples = (
        predict_y_samples(dgp.as_prediction_model(), obs_new, 1000).numpy().T
    )
    ...
    plt.fill_between(
        obs_new[:, 0],
        (pred_f_mean + 2 * np.sqrt(pred_f_var)),
        (pred_f_mean - 2 * np.sqrt(pred_f_var)),
        alpha=0.5,
    )
    ...
    plt.savefig(
        save_dir
        / f"run_{run_number}_rr_{random_restart_number}_{causal_direction}_conditional_gpflux"
    )
    plt.close()
